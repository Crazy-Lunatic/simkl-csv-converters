...
def find_db_in_folder(folder):
    """Find the Tautulli database inside a selected folder."""
    folder = Path(folder)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "When --db is omitted, a folder picker opens. When --user is omitted, "
            "you will be asked to type a username or choose one from the database."
        )
    )
    parser.add_argument(
        '--db',
        default=None,
        help="Path to tautulli.db. If omitted, a folder picker opens."
    )
    parser.add_argument(
        '--output', '-o',
    ...
    if selected_user:
        ...
        print("Exporting watch history for ALL USERS (--all-users was specified).")
